...
fn coordinator_registry_path(repo_root: &Path) -> std::path::PathBuf {
    repo_root.join(crate::COORDINATOR_TASK_REGISTRY_REL_PATH)
}

pub(crate) fn coordinator_pause_file_path(repo_root: &Path) -> std::path::PathBuf {
    repo_root.join(crate::COORDINATOR_PAUSE_FILE_REL_PATH)
}
...
pub(crate) fn clear_coordinator_pause_file(repo_root: &Path) -> Result<bool> {
    let path = coordinator_pause_file_path(repo_root);
    if !path.exists() {
        return Ok(false);
    }
...
pub(crate) fn read_coordinator_pause_file(repo_root: &Path) -> Result<Option<serde_json::Value>> {
    let path = coordinator_pause_file_path(repo_root);
    if !path.exists() {
        return Ok(None);
    }
...
pub(crate) fn cleanup_dead_runtime_tasks_in_registry(
